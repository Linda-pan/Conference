...
<script type="text/javascript">
    ...
    function changeEmail(value, row, index) {
        ...
        if (value == 1) {
            ...
            content.push('<a href="javascript:;"  ');
            content.push('onclick="sent2(' + row.userId + ');return false;" ');
            content.push('>');
            content.push("催促缴费");
            content.push('</a>');
        } else if (value == 3) {
            content.push('<a href="javascript:;"  ');
            content.push('onclick="sent3(' + row.userId + ',\'' + row.isPaymentConfirmed + '\');return false;" ');
            content.push('>');
    ...
    function sent1(userId) {
        $.ajax({
            url: '${ctx}/push/passinform',
            type: 'post',
            datetype: 'json',
            data: {userId: userId},
            success: function (result) {
                ...
                if (result != null) {
                    if (result.status == 'true') {
                        $.scojs_message("通知发送成功", $.scojs_message.TYPE_OK);
                    } else {
    ...
    function sent2(userId) {
        $.ajax({
            url: '${ctx}/push/uploadinform',
            type: 'post',
            datetype: 'json',
            data: {userId: userId},
            success: function (result) {
    ...
    function sent3(userId, result) {
        $.ajax({
            url: '${ctx}/push/readyinform',
            type: 'post',
            datetype: 'json',
            data: {
                userId: userId,
