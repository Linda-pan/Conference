...
public class IndexController extends BaseController {
    ...
    public String login(@RequestParam(required = true) String userName, @RequestParam(required = true) String password, HttpServletResponse response, RedirectAttributesModelMap model) {
        User user = userService.selectByName(userName);

        if (userName != null && user != null) {
            if (!user.getIsEmailConfirmed()) {
                model.addFlashAttribute("message", "邮箱尚未验证");
                return "redirect:/";
            }
    ...
    public String loginByGet(@RequestParam(required = true) String userName, HttpServletResponse response,
                             @RequestParam(required = false) String message, RedirectAttributesModelMap model)
            throws Exception {
        User user = userService.selectByName(userName);
